...
from ``openstarry_code.gateway.rpc`` — see this package's ``__init__.py`` — so every
``from openstarry_code.gateway.rpc import ...`` caller keeps working.

Scope policy is owned by :mod:`openstarry_code.gateway.scopes`. Registrations on
...
from openstarry_code.gateway.guest_rpc_policy import GuestRpcPolicy, GuestRpcPolicyError
...
async def _status(params: Any, ctx: RpcContext) -> dict[str, Any]:
    from openstarry_code.gateway.boot import _boot_time_ms
...
def get_dispatcher() -> RpcRegistry:
    ...
    ``from openstarry_code.gateway.rpc import get_dispatcher`` caller keeps working
    without touching sibling submodules. Sub-module imports that trigger
    handler registration live in ``openstarry_code.gateway.rpc.__init__`` so that
...
def validate_classification(registry: RpcRegistry | None = None) -> None:
    ...
    Called at the end of ``openstarry_code.gateway.rpc.__init__`` once every
    ...
    for name in target.methods():
        ...
        if not is_classified(name):
            raise ScopeDriftError(
                ...
                f"opensquilla/gateway/scopes.py"
            )
