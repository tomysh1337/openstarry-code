...
from openstarry_code.session.context_view import build_compaction_context_records
...
def _estimate_payload_tokens(message: str, transcript: list[Any]) -> int:
    ...
    Uses the shared :func:`openstarry_code.session.tokenizer.estimate_tokens` so
    ...
    *,
    ...
    if session_manager is not None:
        ...
        try:
            ...
            outcome.flush_receipt = await await_compaction_phase(
                _await_auto_summarize_flush_grace(
                    config=config,
