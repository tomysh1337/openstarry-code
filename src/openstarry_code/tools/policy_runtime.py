...
def private_memory_read_tools_blocked(ctx: ToolContext | None) -> bool:
    ...
    from openstarry_code.session.keys import allows_private_memory_prompt_injection
...
def _detect_image_generation_capability() -> bool:
    try:
        from openstarry_code.tools.builtin.media import image_generation_available
...
def detect_runtime_tool_surface_capabilities(
    *,
    ...
    try:
        from openstarry_code.tools.builtin import sessions
        ...
        pass
    try:
        from openstarry_code.tools.builtin import admin
