...
from openstarry_code.provider.types import ChatConfig, derive_provider_request_correlation
...
def _sensitive_media_path_block(tool_name: str, resolved: Path, original_path: str) -> dict | None:
    from openstarry_code.sandbox.sensitive_paths import build_block_envelope, is_sensitive_path
...
def _sensitive_media_url_block(tool_name: str, url: str) -> dict | None:
    from openstarry_code.tools.builtin.web import _sensitive_url_marker
...
async def _fetch_image_url(url: str) -> tuple[bytes, str]:
    ...
    from openstarry_code.tools.ssrf import environment_proxy_url, pinned_transport
...
def _has_explicit_scope_override(scope: str) -> bool:
    return bool(
        os.environ.get(f"OPENSTARRY_CODE_{scope}_PROVIDER")
        or os.environ.get(f"OPENSTARRY_CODE_{scope}_MODEL")
    )
...
def _configured_provider_config(provider_name: str, model: str):
    ...
    return ProviderConfig(
        ...
        api_key=api_key,
        base_url=base_url,
        proxy=proxy or os.environ.get("OPENSTARRY_CODE_LLM_PROXY", ""),
...
def _resolve_provider_config(scope: str, *, default_model: str):
    ...
    provider_name = (
        os.environ.get(f"OPENSTARRY_CODE_{scope}_PROVIDER")
        or os.environ.get("OPENSTARRY_CODE_LLM_PROVIDER")
        or "openrouter"
    )
    model = (
        os.environ.get(f"OPENSTARRY_CODE_{scope}_MODEL")
        or os.environ.get("OPENSTARRY_CODE_LLM_MODEL")
        or default_model
    )
    ...
    return ProviderConfig(
        ...
        api_key=api_key,
        base_url=base_url,
        proxy=os.environ.get("OPENSTARRY_CODE_LLM_PROXY", ""),
    )
