...
from openstarry_code.gateway.routing import build_subagent_route_envelope
from openstarry_code.gateway.session_view import derive_transcript_title
from openstarry_code.provider.auxiliary_budget import resolve_auxiliary_request_budget
from openstarry_code.provider.correlation_context import (
    current_provider_request_correlation,
)
from openstarry_code.provider.types import derive_provider_request_correlation
from openstarry_code.sandbox.run_context import RUN_CONTEXT_ORIGIN_KEY
from openstarry_code.session.keys import build_subagent_session_key, parse_agent_id
from openstarry_code.tools.registry import tool
from openstarry_code.tools.run_mode import current_run_mode, full_host_access_for_context
from openstarry_code.tools.types import PlanAccess, SafeToolError, ToolError, current_tool_context

_log = structlog.get_logger("openstarry_code.tools.sessions")
...
def _run_mode_label(run_mode: str | None) -> str | None:
    if run_mode is None:
        return None
    from openstarry_code.run_mode import display_name
