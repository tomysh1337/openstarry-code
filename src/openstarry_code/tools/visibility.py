...
from openstarry_code.provider.types import ToolDefinition
from openstarry_code.tools.plan_access import plan_access_allows
...
def resolve_profile(ctx: ToolContext | None) -> ToolProfile:
    override = os.environ.get("OPENSTARRY_CODE_TOOL_PROFILE", "").strip()
    if override:
        try:
