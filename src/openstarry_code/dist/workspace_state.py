...
Emits a reproducible, versioned inventory of the current openstarry-code install:

    {
      "schema_version": 1,
      "opensquilla_version": "<pyproject version>",
...
# under ``openstarry_code.channels``. Helpers (``_util``, ``manager``, ``types``)
...
# ``openstarry_code.tools.builtin``. ``shell_policy`` is excluded as a helper
# consumed by ``shell``.
...
def _read_package_metadata() -> tuple[str, str]:
    """Return (version, python_requires) from installed openstarry-code metadata."""

    try:
        meta = _pkg_metadata("openstarry-code")
...
def build_workspace_state() -> dict[str, object]:
    ...
    opensquilla_version, python_requires = _read_package_metadata()
    return {
