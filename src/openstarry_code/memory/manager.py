...
``monkeypatch.setattr("openstarry_code.memory.store.LongTermMemoryStore", FakeStore)``
...
async def build_memory_managers(
    ...
    *,
    ...
    # ``openstarry_code.memory.store.LongTermMemoryStore``,
    # ``openstarry_code.agents.scope.maybe_migrate_legacy_memory``) and have those
    ...
    try:
        for agent_id in agent_ids:
            # Resolve paths
            if agent_id == "main" and os.environ.get("OPENSTARRY_CODE_MEMORY_DB"):
                db_path = Path(os.environ["OPENSTARRY_CODE_MEMORY_DB"])
            else:
            ...
            elif agent_id == "main" and os.environ.get("OPENSTARRY_CODE_MEMORY_DIR"):
                mem_dir = os.environ["OPENSTARRY_CODE_MEMORY_DIR"]
