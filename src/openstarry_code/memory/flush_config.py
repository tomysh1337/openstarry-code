...
configuration explicitly enables it, ``OPENSTARRY_CODE_SESSION_FLUSH`` remains a
...
def is_session_flush_enabled() -> bool:
    """Return False iff OPENSTARRY_CODE_SESSION_FLUSH is explicitly disabled."""
    raw = os.environ.get("OPENSTARRY_CODE_SESSION_FLUSH")
    if raw is None:
        return True
