...
if TYPE_CHECKING:
    from openstarry_code.channels.types import ManagedChannel
...
def discover_channel_names() -> list[str]:
    ...
    import openstarry_code.channels as pkg
...
def discover_plugins() -> dict[str, ChannelRegistration]:
    ...
    for ep in entry_points(group="openstarry_code.channels"):
        try:
...
def _discover_builtin(module_name: str) -> ChannelRegistration | None:
    module = importlib.import_module(f"openstarry_code.channels.{module_name}")
...
def _resolve_entry_model(module_name: str, module: Any) -> type[BaseModel] | None:
    ...
    gateway_config = importlib.import_module("openstarry_code.gateway.config")
