...
Note: distinct from ``openstarry_code.engine.pipeline.TurnContext`` which is
the pre-turn pipeline value object. The two coexist while the pipeline
TurnContext stays in place. Import this one as::

    from openstarry_code.engine.turn_runner.context import TurnContext as HarnessTurnContext
