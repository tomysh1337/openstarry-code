...
from openstarry_code.session.compaction_lifecycle import flush_receipt_is_successful_flush
...
async def run_memory_flush_session(
    *,
    ...
    from openstarry_code.cli.agent_cmd import _with_agent_workspace_config
    from openstarry_code.gateway import build_services
    ...
    try:
        ...
        if flush_service is None:
            raise RuntimeError(
                ...
                "and leave OPENSTARRY_CODE_SESSION_FLUSH unset before running memory flush-session"
            )
...
def memory_flush_session_cmd(
    ...
    config_path: str = typer.Option(
        ...
        "--config",
        help=(
            "Gateway config path. Defaults to OPENSTARRY_CODE_GATEWAY_CONFIG_PATH "
