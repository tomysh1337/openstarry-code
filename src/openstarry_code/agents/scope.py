...
def default_opensquilla_home() -> Path:
    """Return the user-level OpenStarry Code home directory.

    Delegates to :func:`openstarry_code.paths.default_opensquilla_home` so the
    ``OPENSTARRY_CODE_STATE_DIR`` env override is honored uniformly.
    """
    from openstarry_code.paths import default_opensquilla_home as _impl
...
def resolve_agent_workspace_dir(
    ...
    *,
    ...
    When no workspace is configured, fall back to OpenStarry Code's user-level
