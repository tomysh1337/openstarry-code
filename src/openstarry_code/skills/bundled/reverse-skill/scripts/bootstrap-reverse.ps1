...
param(
    ...
    [string]$PipIndexUrl = $env:REVERSE_PIP_INDEX_URL,
...
if ([string]::IsNullOrWhiteSpace($PipIndexUrl)) {
    $PipIndexUrl = [Environment]::GetEnvironmentVariable('REVERSE_PIP_INDEX_URL', 'User')
}
...
function Get-UsablePythonInvocation {
    try {
        $spec = Resolve-ReverseToolSpec -Name 'python'
        if ($spec.Available -and -not [string]::IsNullOrWhiteSpace([string]$spec.Command)) {
            & $spec.Command @($spec.PrefixArgs) --version *> $null
            if ($LASTEXITCODE -eq 0) {
                return [pscustomobject]@{ Command = $spec.Command; PrefixArgs = @($spec.PrefixArgs) }
            }
        }
    }
    catch {
    ...
    foreach ($name in @('python', 'python3')) {
        $path = Get-FirstCommandPath -Names @($name) -PreferApplication
        if ([string]::IsNullOrWhiteSpace($path)) {
            continue
        }
        & $path --version *> $null
        if ($LASTEXITCODE -eq 0) {
            return [pscustomobject]@{ Command = $path; PrefixArgs = @() }
        }
    }

    $py = Get-FirstCommandPath -Names @('py.exe', 'py') -PreferApplication
    if (-not [string]::IsNullOrWhiteSpace($py)) {
        foreach ($selector in @('-3.12', '-3')) {
            & $py $selector --version *> $null
            if ($LASTEXITCODE -eq 0) {
                return [pscustomobject]@{ Command = $py; PrefixArgs = @($selector) }
            }
        }
    }
...
function Ensure-PythonRuntime {
    if ($null -ne (Get-UsablePythonInvocation)) {
        return
    }

    Ensure-WingetPackage -Id 'Python.Python.3.13' -Label 'Python 3.13'
    if ($null -eq (Get-UsablePythonInvocation)) {
        throw 'Python was installed, but no usable interpreter passed the version check.'
    }
}
...
function Ensure-PipPackageInstall {
    ...
    $python = Get-UsablePythonInvocation
    if ($null -eq $python) {
        throw 'No usable Python interpreter is available for pip installation.'
    }
    ...
    $pipArguments = @($python.PrefixArgs) + @(
        '-m', 'pip', 'install', '--upgrade', '--disable-pip-version-check',
        '--no-input', '--prefer-binary', '--timeout', '180', '--retries', '10'
    )
    if (-not [string]::IsNullOrWhiteSpace($PipIndexUrl)) {
        $pipArguments += @('--index-url', $PipIndexUrl)
    }
    $pipArguments += $installTarget

    & $python.Command @pipArguments
    if ($LASTEXITCODE -ne 0) {
...
foreach ($name in $expandedCapabilities) {
    ...
    try {
        ...
        $resultStatus = if ($state -and -not $state.Ready) { 'installed-not-ready' } else { 'ready' }
        $results += [pscustomobject]@{
            name = $name
            status = $resultStatus
