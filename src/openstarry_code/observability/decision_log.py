...
``~/.openstarry-code/logs/decisions-YYYYMMDD.jsonl`` at the end of every completed turn.
...
When ``OPENSTARRY_CODE_DEBUG_LOG=1``, a mirror of the structured entry is written
under ``~/.openstarry-code/logs/debug/`` for operator debugging.
"""
...
def _default_log_dir() -> Path:
    ...
    Honours the ``OPENSTARRY_CODE_LOG_DIR`` env override; defaults to the user-level
    ``~/.openstarry-code/logs`` directory.
    """

    return Path(os.environ.get("OPENSTARRY_CODE_LOG_DIR", str(default_opensquilla_home() / "logs")))
...
def write_decision_entry(
    ...
    # Debug mirror: opt-in via OPENSTARRY_CODE_DEBUG_LOG=1. Mirrors the *structured*
    ...
    if os.environ.get("OPENSTARRY_CODE_DEBUG_LOG") == "1":
