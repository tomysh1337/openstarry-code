"""FastMCP server factory for exposing OpenStarry Code session workflows."""
...
def create_mcp_server(
    ...
    *,
    name: str = "OpenStarry Code",
    fastmcp_cls: type[Any] | None = None,
) -> Any:
    """Create a FastMCP app with product-oriented OpenStarry Code tools/resources."""
    ...
    if fastmcp_cls is None:
        try:
            ...
            raise RuntimeError(
                "The MCP server requires the optional dependency: install openstarry-code[mcp]."
            ) from exc
        fastmcp_cls = FastMCP
    ...
    async def conversations_list(limit: int = 50) -> dict[str, Any]:
        """List OpenStarry Code sessions visible to the connected gateway principal."""
    ...
    async def session_resolve(key: str) -> dict[str, Any]:
        """Resolve a session key or identifier to OpenStarry Code session metadata."""
    ...
    async def messages_read(key: str, limit: int = 1000) -> dict[str, Any]:
        """Read persisted messages for an OpenStarry Code session."""
    ...
    async def messages_send(key: str, message: str, intent: str = "continue") -> dict[str, Any]:
        """Send a user message to an existing OpenStarry Code session."""
    ...
    async def events_wait(
        ...
        """Wait for live or replayed gateway events for an OpenStarry Code session."""
