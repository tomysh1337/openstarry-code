...
# Operator escape hatch — set OPENSTARRY_CODE_SENSITIVE_PATHS_DISABLED=1 to no-op
...
_DISABLED = os.environ.get(
    "OPENSTARRY_CODE_SENSITIVE_PATHS_DISABLED", ""
...
def is_sensitive_path(path: str) -> str | None:
    ...
    Honors :data:`_DISABLED` (env var ``OPENSTARRY_CODE_SENSITIVE_PATHS_DISABLED``).
    """
    if _DISABLED:
        return None
    if not path:
...
def sensitive_path_marker(
    ...
    *,
    ...
    Container deployments commonly place OpenStarry Code's default workspace under
    ``/root/.openstarry-code/workspace``. The broad ``/root`` deny prefix should not
...
def sensitive_target_in_command(
    ...
    *,
    ...
    if _DISABLED:
        return None
    from openstarry_code.sandbox.destructive_intents import _extract_intents
