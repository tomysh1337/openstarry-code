...
function Install-WindowsVCRedistIfNeeded {
    ...
    if ($winget) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host 'OpenStarry Code: Microsoft Visual C++ Redistributable installation completed.'
            return $true
        }
        Write-Warning (
...
if (-not $env:OPENSTARRY_CODE_GATEWAY_WORKSPACE_DIR) {
    $env:OPENSTARRY_CODE_GATEWAY_WORKSPACE_DIR = Join-Path $env:OPENSTARRY_CODE_STATE_DIR 'workspace'
}
