...
def test_channel_dispatch_no_longer_uses_gateway_cron_intent_preflight() -> None:
    source = Path("src/openstarry_code/gateway/channel_dispatch.py").read_text(
        encoding="utf-8"
    )
...
def test_gateway_cron_intent_module_removed_from_runtime_surface() -> None:
    assert not Path("src/openstarry_code/gateway/channel_cron_intent.py").exists()
...
async def test_schedule_like_channel_text_reaches_normal_runtime() -> None:
    ...
    with (
        ...
        patch(
            "openstarry_code.gateway.channel_dispatch._should_skip_unmentioned",
        ...
        patch(
            "openstarry_code.gateway.channel_dispatch._status_reactor",
